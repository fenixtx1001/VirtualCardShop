...
function parseCardNumberSortKey(cardNumber: string) {
  ...
  // Put hyphenated / checklist-style numbers at the end, but stable.
  if (s.includes("-")) {
...
function displayToMoney(s: string) {
  // allow "", ".", "0.", etc. while typing; parse safely on blur/save
  const n = Number(s);
...
export default function ProductSetDetailClient({ productSetId }: { productSetId: string }) {
  ...
  const [insertFilter, setInsertFilter] = useState("ALL");
  const [variantFilter, setVariantFilter] = useState("ALL");

  // ✅ Draft strings so Book inputs behave like normal textboxes while typing
  ...
  async function load() {
    ...
    try {
      ...
      // Initialize draft values so inputs show correct text without fighting the user
      const nextDraft: Record<number, string> = {};
      for (const c of payload.cards ?? []) {
        nextDraft[c.id] = moneyToDisplay(c.bookValue ?? 0);
      }
  ...
  const filterOptions = useMemo(() => {
    ...
    for (const c of cards) {
      ...
      inserts.add(normalizeOpt(c.insert));
    ...
    return {
      ...
      inserts: Array.from(inserts).sort(sortAlpha),
  ...
  const filteredCards = useMemo(() => {
    ...
    return sortedCards.filter((c) => {
      if (subsetFilter !== "ALL") {
        if (normalizeOpt(c.subset) !== subsetFilter) return false;
      }
      if (insertFilter !== "ALL") {
        if (normalizeOpt(c.insert) !== insertFilter) return false;
      }
      if (variantFilter !== "ALL") {
        if (normalizeOpt(c.variant) !== variantFilter) return false;
      }
  ...
  }, [sortedCards, query, subsetFilter, insertFilter, variantFilter]);
    ...
    try {
      ...
      const res = await fetch(`/api/cards/${encodeURIComponent(String(card.id))}`, {
        ...
        body: JSON.stringify({
          ...
          bookValue, // ✅ numeric, derived from draft if present
          productSetId, // keep it attached
          imageUrl: card.imageUrl ?? null, // ✅ NEW (and safe)
        }),
      });
      ...
      patchCard(card.id, { bookValue, imageUrl: card.imageUrl ?? null });
  ...
  async function deleteCard(card: CardRow) {
    ...
    try {
      ...
      setData((prev) => {
        if (!prev) return prev;
        return { ...prev, cards: prev.cards.filter((c) => c.id !== card.id) };
      });
      ...
      {loading || !data ? (
        ...
        <>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <div style={{ fontWeight: 700 }}>Insert</div>
              <select value={insertFilter} onChange={(e) => setInsertFilter(e.target.value)} style={{ padding: 8, width: 220 }}>
                <option value="ALL">All</option>
                {filterOptions.inserts.map((v) => (
                  <option key={v} value={v}>
                    {v}
                  </option>
                ))}
          ...
          <div style={{ overflowX: "auto", border: "1px solid #ddd" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead style={{ position: "sticky", top: 0, background: "#f7f7f7", zIndex: 2 }}>
                <tr>
                  {["Row", "Card #", "Player", "Team", "Subset", "Insert", "Variant", "Image URL", "Qty", "Book", "Actions"].map((h) => (
                  ...
                  return (
                    <tr key={c.id} style={{ background: zebra }}>
                      ...
                      <td style={{ ...bodyCell, minWidth: 160 }}>
                        <input
                          value={c.insert ?? ""}
                          onChange={(e) => patchCard(c.id, { insert: e.target.value || null })}
                          style={{ width: "100%", padding: 6 }}
                        />
                      </td>

                      <td style={{ ...bodyCell, minWidth: 140 }}>
                        <input
                          value={c.variant ?? ""}
                      ...
                      {/* ✅ NEW: Image URL */}
                      <td style={{ ...bodyCell, minWidth: 360 }}>
                        <input
                          value={c.imageUrl ?? ""}
                          onChange={(e) => patchCard(c.id, { imageUrl: e.target.value || null })}
                          placeholder="https://..."
                          style={{ width: "100%", padding: 6 }}
                        />
                      </td>
                      ...
                      {/* ✅ Book uses draft string so typing works */}
                      <td style={{ ...bodyCell, width: 110 }}>
                        <input
