...
export async function PUT(req: Request, ctx: Ctx) {
  ...
  const updated = await prisma.product.update({
    ...
    data: {
      ...
      packPriceCents:
        body?.autoPackPricing === false
          ? body?.packPriceCents ?? undefined
          : undefined,

      autoPackPricing: boolOrUndefined(
        body?.autoPackPricing
      ),
      ...
      released: boolOrUndefined(body?.released),
    },
  });

  const product = updated.autoPackPricing
    ? await repriceProduct(updated.id)
    : updated;
