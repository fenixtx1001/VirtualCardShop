...
ROOKIE_SENSATIONS_URL = "https://www.tcdb.com/Checklist.cfm/sid/2150/1993-94-Fleer-Rookie-Sensations"
SHARPSHOOTERS_URL = "https://www.tcdb.com/Checklist.cfm/sid/2151/1993-94-Fleer-Sharpshooters"
TOWER_OF_POWER_URL = "https://www.tcdb.com/Checklist.cfm/sid/2152/1993-94-Fleer-Tower-of-Power"
...
def parse_rows(html: str, number_pattern: str) -> list[tuple[str, str, str]]:
    ...
    for row in parser.rows:
        ...
        for i, value in enumerate(vals):
            ...
            if not pattern.fullmatch(number) or i + 1 >= len(vals):
                continue
...
ODDS_TEXT = re.compile(
    ...
    r"\d+\s*:\s*\d+\s*(?:series\s+(?:one|two)\s+)?"
    r"(?:wax\s+|cello\s+|jumbo\s+)?(?:packs?)?",
    re.IGNORECASE,
)
...
def build_base(true_rcs: set[int]) -> list[list[str]]:
    ...
    for number in range(1, 401):
        ...
        if number in true_rcs:
            ...
            player = f"{player} RC"
        rows.append(["base", str(number), player, team, subset, "UER" if had_uer else ""])
    return rows


def build_standard_insert(
    *, key: str, url: str, expected: int, subset: str
) -> list[list[str]]:
...
def build_clyde_highlights() -> list[list[str]]:
    source = fetch_numbered_set(
        CLYDE_URL, 16, "clyde-career-highlights-source", max_pages=2
    )
    rows: list[list[str]] = []
    for number in range(1, 13):
        ...
        title = re.sub(
            r"^Clyde\s+Drexler\b", "", clean, flags=re.IGNORECASE
        ).strip(" -–—")
        rows.append([
            "clyde-career-highlights", str(number), "Clyde Drexler", team,
            "Clyde Drexler Career Highlights", title,
        ])
    return rows
...
def build_clyde_autographs() -> list[list[str]]:
    source = fetch_numbered_set(
        CLYDE_AUTO_URL, 12, "clyde-career-highlights-autographs", max_pages=2
    )
    rows: list[list[str]] = []
    for number in range(1, 13):
        ...
        rows.append([
            "clyde-career-highlights-autographs", str(number), "Clyde Drexler", team,
            "Clyde Drexler Career Highlights Autographs", "AU",
        ])
    return rows
...
def validate_rows(rows: list[list[str]], true_rcs: set[int]) -> None:
    ...
    for key, expected in EXPECTED_COUNTS.items():
        if counts.get(key, 0) != expected:
            raise SystemExit(
                f"{key}: expected {expected} rows, found {counts.get(key, 0)}"
            )
    ...
    checks = {
        ...
        ("rookie-sensations", "1"): ("Anthony Avent", "Milwaukee Bucks"),
        ...
        ("tower-of-power", "1"): ("Charles Barkley", "Phoenix Suns"),
        ("tower-of-power", "21"): ("Shaquille O'Neal", "Orlando Magic"),
        ("tower-of-power", "29"): ("Chris Webber", "Golden State Warriors"),
    }
    ...
    if lookup[("clyde-career-highlights", "1")][2] != "Clyde Drexler":
        raise SystemExit("Clyde Career Highlights #1 Player normalization failed")
    ...
    for row in rows:
        ...
        seen.add(key)
    if duplicates:
        raise SystemExit(f"Duplicate ProductSet/card numbers: {sorted(duplicates)[:10]}")

    metadata_leaks = [
        row for row in rows
        if re.search(
            r"\b(?:UER|ERR|COR|VAR|RDM|EXCH|SN\d+|PR\d+)\b|"
            r"\bprint\s+code\b|\b\d+\s*:\s*\d+\b",
    ...
    odds_leaks = [
        row for row in rows
        if re.search(r"\b\d+\s*:\s*\d+\b", row[5] or "")
    ]
    if odds_leaks:
...
def main() -> None:
    ...
    rows.extend(build_standard_insert(
        key="all-stars", url=ALL_STARS_URL, expected=24, subset="All-Stars"
    ))
    rows.extend(build_clyde_highlights())
    rows.extend(build_clyde_autographs())
    rows.extend(build_standard_insert(
        key="nba-internationals", url=INTERNATIONALS_URL,
        expected=12, subset="NBA Internationals"
    ))
    rows.extend(build_standard_insert(
        key="rookie-sensations", url=ROOKIE_SENSATIONS_URL,
        expected=24, subset="Rookie Sensations"
    ))
    rows.extend(build_standard_insert(
        key="first-year-phenoms", url=FIRST_YEAR_URL,
        expected=10, subset="First Year Phenoms"
    ))
    rows.extend(build_standard_insert(
        key="living-legends", url=LIVING_LEGENDS_URL,
        expected=6, subset="Living Legends"
    ))
    rows.extend(build_standard_insert(
        key="nba-superstars", url=SUPERSTARS_URL,
        expected=20, subset="NBA Superstars"
    ))
    rows.extend(build_standard_insert(
        key="sharpshooters", url=SHARPSHOOTERS_URL,
        expected=10, subset="Sharpshooters"
    ))
    rows.extend(build_standard_insert(
        key="tower-of-power", url=TOWER_OF_POWER_URL,
        expected=30, subset="Tower of Power"
    ))
    ...
    print("Rookie Sensations: 24 cards, verified 1:5 Series One cello packs")
    print("Tower of Power: 30 cards, verified 2:3 Series Two cello packs")
    print("Tower of Power gameplay odds: pending fractional/multi-hit odds support")
    print("NBA Superstars / Sharpshooters: ProductSet odds pending verification")
    print("Clyde Highlights #13-16: excluded as non-pack distributions")
    print("Mail-in / promo issues: excluded")
